...
def grade_by_logic_barem(project_data, de_thi):
    ...
    # 1. Kiểm tra gán biến Trả lời = Có (0.5đ)
    ...
    if txt1 in chuan_hoa(code_str): total_score += 0.5; report.append(f"✅ 8. Thông báo đúng 1 (0.5đ)")
    else: report.append("❌ 8. Sai thông báo 1 (0đ)")
    if txt2 in chuan_hoa(code_str): total_score += 0.5; report.append(f"✅ 9. Thông báo đúng 2 (0.5đ)")
    else: report.append("❌ 9. Sai thông báo 2 (0đ)")

    # 10. Tiếp tục (0.5đ) & 11. Kết thúc (0.5đ)
...
# --- GIAO DIỆN ---
st.set_page_config(page_title="Hệ thống chấm thi", page_icon="🏫")
    ...
    if st.button("NỘP BÀI"):
        if ten_hs and file_sb3:
            try:
                ...
                pd.concat([existing, new_row], ignore_index=True).pipe(conn.update)
...
with tab2:
    ...
    if pwd == MAT_KHAU_GV:
        # Quan trọng: Dùng ttl=0 để luôn lấy dữ liệu mới nhất
