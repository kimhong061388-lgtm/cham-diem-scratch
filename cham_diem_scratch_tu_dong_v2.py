...
with st.sidebar:
    ...
    st.warning("⚠️ **QUY ĐỊNH:** Chấm điểm dựa trên tên biến và công thức chính xác.")
    st.divider()
...
# !!! BẠN HÃY DÁN LINK WEBHOOK CỦA BẠN VÀO ĐÂY !!!
...
# --- 3. HÀM CHẤM ĐIỂM "QUÉT TOÀN DIỆN" ---
def grade_by_logic_barem(project_data, de_thi):
    ...
    # BƯỚC 1: THU THẬP TẤT CẢ BIẾN VÀ KHỐI LỆNH TRÊN TOÀN BỘ PROJECT
    for target in project_data.get('targets', []):
        # Lấy biến
        for v_id, v_info in target.get('variables', {}).items():
            var_id_to_name[v_id] = chuan_hoa(v_info[0])
        # Lấy khối lệnh
    ...
    # 3+4. NHẬP LIỆU (Kiểm tra biến gán với Answer)
    vars_receiving_answer = set()
        ...
        if b.get('opcode') == 'data_setvariableto':
            ...
            if 'sensing_answer' in val_input:
                ...
                if v_id in var_id_to_name:
                    vars_receiving_answer.add(var_id_to_name[v_id])

    if req_in1 in vars_receiving_answer and req_in2 in vars_receiving_answer:
        total_score += 1.0
        report.append(f"✅ 3+4. Nhập đủ 2 biến {req_in1.upper()}, {req_in2.upper()} (1.0đ)")
    else:
        report.append(f"❌ 3+4. Sai tên biến nhập liệu (Yêu cầu: {req_in1.upper()}, {req_in2.upper()}) (0đ)")

    # 5. CÔNG THỨC CHÍNH XÁC (Bắt lỗi gán đè và thứ tự)
    formula_ok = False
    for b in all_blocks.values():
        if b.get('opcode') == 'data_setvariableto':
            v_res_id = b.get('fields', {}).get('VARIABLE', [None])[0]
            v_res_name = var_id_to_name.get(v_res_id, "")
            
            # Nếu biến nhận kết quả đúng (I hoặc V)
            if v_res_name == req_out:
                ...
                if isinstance(val_input, list) and len(val_input) > 1:
                    ...
                    if child and child.get('opcode') == 'operator_divide':
                        # Hàm lấy tên biến từ NUM1, NUM2
                        def get_var_from_input(inp_key):
                            ...
                            if isinstance(inp_data, list) and len(inp_data) > 1:
                                ...
                                if sub_b and sub_b.get('opcode') == 'data_variable':
                                    return var_id_to_name.get(sub_b.get('fields', {}).get('VARIABLE', [None])[0], "")
                            return ""

                        n1_name = get_var_from_input('NUM1')
                        n2_name = get_var_from_input('NUM2')

                        if n1_name == req_in1 and n2_name == req_in2:
    ...
    if formula_ok:
        ...
        report.append("❌ 5. Sai công thức, tên biến hoặc gán đè (0đ)")
        script_desc.append("[Toán: SAI]")

    # 6 & 7. If-Else và Ngưỡng (0.5đ + 0.5đ)
