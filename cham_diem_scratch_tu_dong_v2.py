...
st.title("🏆 Hệ thống Thi Scratch V2 (Tự động lưu)")
...
if st.button("NỘP BÀI"):
    if ten_hs and file_sb3:
        try:
            ...
            # --- XỬ LÝ LƯU ĐIỂM TỰ ĐỘNG ---
            FORM_ID = "1FAIpQLSe0-1jZxmGAN1ZAh-FECB-csQuXUEIcRv2n9kLErJQP5hQM_w"
            base_url = f"https://google.com{FORM_ID}/formResponse"
            params = {
                "entry.1207556323": datetime.now().strftime("%H:%M:%S %d/%m/%Y"),
                "entry.1491419959": ten_hs, "entry.102516187": lop_hs,
                "entry.31627165": de_thi, "entry.168917856": str(score), "submit": "Submit"
            }
            
            # Thử gửi ngầm
            try:
                requests.post(base_url, data=params, timeout=2)
                st.success("✅ Đã tự động ghi nhận điểm!")
            except:
                # Nếu mạng trường chặn, hiện nút bấm sạch sẽ
                st.warning("👉 Hãy nhấn nút dưới đây để XÁC NHẬN lưu điểm:")
                query = urllib.parse.urlencode(params)
                st.link_button("XÁC NHẬN LƯU ĐIỂM", f"{base_url}?{query}")
            
            for d in details: st.write(d)
            if score == 6.0: st.balloons()
            
            # TẢI PHIẾU ĐIỂM MINH CHỨNG
